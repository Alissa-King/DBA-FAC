...
-----
  ...
  # Pin a specific held-out year:
  python fac_model.py --input ./fac_data/analytic_audit_level.parquet \
                      --test-year 2023 --results-out results.json

OUTPUT JSON SCHEMA (when --results-out is used)
----------------------------------------------
  target            str    column being predicted
  train_years       list   years used for training
  test_year         int    held-out year
  target_recall     float  recall threshold used
  test_base_rate    float  fraction of positives in the test year
  models            list   one entry per model: name, auc, ap, brier,
                           precision, recall, f1, threshold, tp, fp, fn, tn
  best_model        dict   same schema as one model entry
  top_features      list   {feature, importance, method} dicts
  calibration       list   {mean_pred, frac_pos} dicts (5 quantile bins)
  feature_sets      dict   lists of columns used in each feature set
...
def pick_threshold_for_recall(y_true, scores, target_recall=0.80):
    ...
    decision threshold to hit a recall target, then report the precision we
    pay for it. (A finance team would rather over-flag than miss a finding.)
    """
...
def get_calibration(y_te, scores, bins=5):
    """
    Returns calibration bins as a list of dicts for JSON output and dashboard
    display. Each bin holds the average predicted probability and the actual
    fraction of positives in that bin.
    """
...
def run(args):
    ...
    # ---- RQ2: calibration (best model overall) ----
...
def parse_args():
    ...
    p.add_argument("--test-year", type=int, default=None,
                   help="Held-out future year. Defaults to the latest year in the data.")
    p.add_argument("--results-out", default=None,
                   help="Write metrics JSON here (for the dashboard and committee review).")
    return p.parse_args()
