...
-----
  ...
  # Quick bounded test (1 page per endpoint/year, good for verifying auth):
  python fac_ingest.py --start-year 2022 --end-year 2023 --max-pages 1 --out ./fac_pilot
...
def build_analytic_table(general_df, awards_df, findings_df):
    """
    Audit-level analytic table: one row per organization-year.

    Data-quality handling:
      * Validates material-weakness flag against the findings endpoint
        (not the general table flag, which was mis-mapped in migration
        per FAC documented concern #9).
      * Uses fiscal-year fields for time splitting, not fac_accepted_date
        (off-by-one for migrated pre-2016 records, FAC concern #3).
    """
    gen = general_df.copy()
    ...
    for col in ["any_material_weakness", "any_significant_deficiency",
                "any_questioned_costs", "any_repeat_finding"]:
...
def run(args):
    ...
    print("\n  Next step: run fac_model.py on this analytic table.")
